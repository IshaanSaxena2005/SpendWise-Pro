...
import { addTransaction, getCategories, type Transaction } from '../../lib/store';
...
export function AddTransactionModal({ isOpen, onClose, editTxn }: Props) {
  ...
  const [catId, setCatId]   = useState(editTxn?.category_id?.toString() || String(categories[0]?.id || '1'));
      ...
      <div className="relative bg-white rounded-2xl w-full max-w-md p-6 shadow-2xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex gap-3 pt-1">
            ...
            <button
              type="submit"
              className="flex-1 py-2.5 rounded-xl bg-black text-white text-sm font-medium flex items-center justify-center gap-2 hover:bg-gray-800 transition-colors"
            >
