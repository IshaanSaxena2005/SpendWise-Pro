...
import { addExpenseApi, updateExpenseApi } from '../../lib/expenses';
...
function TransactionForm({ editTxn, onClose, onSaved }: FormProps) {
  const categories = useCategories();
  const [type, setType]     = useState<'expense' | 'income'>(editTxn?.type ?? 'expense');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editTxn) {
        await updateExpenseApi(editTxn.id, payload);
      } else {
    ...
    <>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex gap-3 pt-1">
          ...
          >
            {saving ? 'Saving…' : 'Save'} <ArrowRight className="w-4 h-4" />
          </button>
        </div>
      </form>
...
export function AddTransactionModal({ isOpen, onClose, editTxn, onSaved }: Props) {
